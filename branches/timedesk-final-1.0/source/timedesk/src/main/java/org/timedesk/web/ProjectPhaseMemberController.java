...
public class ProjectPhaseMemberController {
    ...
    public String createForm(Model model, @RequestParam(value = "parentId", required = false) Long parentId) {
        ...
        if (parentId != null) {
            ...
            if (projectPhase != null) phaseMember.setProjectPhase(projectPhase);
        }
    ...
    public String create(@Valid ProjectPhaseMember projectPhaseMember, BindingResult result, Model model, HttpServletRequest request) {
        ...
        if (phaseMemberId != null) projectPhaseMember.setPhaseMemberId(phaseMemberId); else {
        ...
        sendMessage("noreply@timedeskonline.com", "TimeDeskOnline Notification: "+projectPhaseMember.getProjectPhase().getProject().getName()+" - Phase: "+projectPhaseMember.getProjectPhase().getVersion(), projectPhaseMember.getProjectMember().getEmployee().getEmail(), "You have been assigned to "+projectPhaseMember.getProjectPhase().getProject().getName()+" - Phase: "+projectPhaseMember.getProjectPhase().getVersion()+" From "+new Date());
    ...
    public Collection<ProjectMember> populateProjectMembers(@RequestParam(value = "parentId", required = false) Long parentId) {
        if (parentId == null) {
            ...
            ProjectPhase projectPhase = ProjectPhase.findProjectPhase(parentId);
            if (projectPhase != null) return ProjectMember.findProjectMemberByProject(projectPhase.getProject().getId()); else return null;
        }
    }
